...
def main():
    ...
    results_borda = {}
    results_borda_appeared = {x:0 for x in range(101)}
    results_copeland = {}
    results_copeland_appeared = {x:0 for x in range(101)}
    ...
    for run in range(runs):
        for percentage in percentages:
            ...
            grid = Grid(12, districts, perc, True, False)
            ...
            if results_plurality_appeared[round_true_perc] > 0:
                appears = results_plurality_appeared[round_true_perc]
                avg_gerry_perc = ((results_plurality[round_true_perc] * appears) + round_gerry_perc) / float(appears + 1)

            results_plurality[round_true_perc] = avg_gerry_perc
            results_plurality_appeared[round_true_perc] += 1
            ...
            round_gerry_perc = int(round(x[1] * 100)) 
            avg_gerry_perc = round_gerry_perc
            if results_borda_appeared[round_true_perc] > 0:
                appears = results_borda_appeared[round_true_perc]
                avg_gerry_perc = ((results_borda[round_true_perc] * appears) + round_gerry_perc) / float(appears + 1)

            results_borda[round_true_perc] = avg_gerry_perc
            results_borda_appeared[round_true_perc] += 1
            ...
            round_gerry_perc = int(round(x[1] * 100)) 
            avg_gerry_perc = round_gerry_perc
            if results_copeland_appeared[round_true_perc] > 0:
                appears = results_copeland_appeared[round_true_perc]
                avg_gerry_perc = ((results_copeland[round_true_perc] * appears) + round_gerry_perc) / float(appears + 1)

            results_copeland[round_true_perc] = avg_gerry_perc
            results_copeland_appeared[round_true_perc] += 1

    results_plurality = collections.OrderedDict(sorted(results_plurality.items()))
    results_borda = collections.OrderedDict(sorted(results_borda.items()))
    results_copeland = collections.OrderedDict(sorted(results_copeland.items()))

    plt.figure()
    plt.title('Gerrymandering results without proportionality contraint, with hotspots')
    plt.plot(results_plurality.keys(), results_plurality.values(), 'r-o', label="plurality")
    plt.plot(results_borda.keys(), results_borda.values(), 'b-o', label="borda")
    plt.plot(results_copeland.keys(), results_copeland.values(), 'g-o', label="copeland")
    plt.xlabel('Actual percentage')
    plt.ylabel('Gerrymandered percentage')
    plt.legend(loc="upper left")
    plt.savefig("results/graphs/allrules.png")
