...
class Rule:
    def __init__(self, grid, gerrying_alt):
        self.text = 'Empty'
        self.grid = grid
        self.gerrying_alt = gerrying_alt
    ...
    def gerry(self):
        ...
        max_iterations = 300
        iteration = 0
        while found_neighbour and (not self.victory(conquer)) and (iteration < max_iterations):
            found_neighbour, new_district, old_district, last_voter = self.step(new_district, old_district)
            iteration += 1
            if (iteration % 100) == 0 and (not self.victory(conquer)):
    ...
    def can_conquer(self):
        points = self.calc_score(self.grid.profile())[self.gerrying_alt]
    ...
    def get_winner_dist_neigh(self, neighbour):
        return self.get_winner_dist(self.grid.dist_list[neighbour.get_district()])
...
class Plurality(Rule):
    ...
    def get_neighbours_by_type(self, neighbours, dist_no, last_dist_no):
        ...
        if dist.get_conquer():
            ...
            neighbours_by_type.append([neighbour for neighbour in neighbours if neighbour.get(1) != 'a' and neighbour.get(1) != self.get_winner_dist(dist)
                and self.to_be_conquered(neighbour)])
            neighbours_by_type.append([neighbour for neighbour in neighbours if neighbour.get(1) != 'a' and neighbour.get(1) != self.get_winner_dist(dist)
                and not self.to_be_conquered(neighbour)])
            neighbours_by_type.append([neighbour for neighbour in neighbours if neighbour.get(1) != 'a' and neighbour.get(1) == self.get_winner_dist(dist)
            ...
            neighbours_by_type.append([neighbour for neighbour in neighbours if neighbour.get(1) != 'a'
                and self.get_winner_dist_neigh(neighbour) == neighbour.get(1)
                and self.to_be_conquered(neighbour)])
            neighbours_by_type.append([neighbour for neighbour in neighbours if neighbour.get(1) != 'a'
                and self.get_winner_dist_neigh(neighbour) != neighbour.get(1)
                and self.to_be_conquered(neighbour)])
...
class Borda(Rule):
    ...
    def get_neighbours_by_type(self, neighbours, dist_no, last_dist_no):
        ...
        if dist.get_conquer():
            ...
            neighbours_by_type.append([neighbour for neighbour in neighbours if neighbour.get(1) == 'a' 
                and self.to_be_conquered(neighbour)])
            neighbours_by_type.append([neighbour for neighbour in neighbours if neighbour.get(2) == 'a' and neighbour.get(1) != self.get_winner_dist(dist)
                and not self.to_be_conquered(neighbour)])
            neighbours_by_type.append([neighbour for neighbour in neighbours if neighbour.get(2) == 'a' and neighbour.get(1) != self.get_winner_dist(dist)
                and self.to_be_conquered(neighbour)])
            neighbours_by_type.append([neighbour for neighbour in neighbours if neighbour.get(2) == 'a' and neighbour.get(1) == self.get_winner_dist(dist)
                and not self.to_be_conquered(neighbour)])
            neighbours_by_type.append([neighbour for neighbour in neighbours if neighbour.get(3) == 'a' and neighbour.get(1) != self.get_winner_dist(dist)
                and self.to_be_conquered(neighbour)])
            neighbours_by_type.append([neighbour for neighbour in neighbours if neighbour.get(2) == 'a' and neighbour.get(1) == self.get_winner_dist(dist)
                and self.to_be_conquered(neighbour)])
            neighbours_by_type.append([neighbour for neighbour in neighbours if neighbour.get(3) == 'a' and neighbour.get(1) != self.get_winner_dist(dist)
            ...
            neighbours_by_type.append([neighbour for neighbour in neighbours if neighbour.get(2) == 'a'
                ...
                and self.to_be_conquered(neighbour)])
            neighbours_by_type.append([neighbour for neighbour in neighbours if neighbour.get(1) == 'a'
                and self.get_winner_dist_neigh(neighbour) == neighbour.get(2)
                and self.to_be_conquered(neighbour)])
            neighbours_by_type.append([neighbour for neighbour in neighbours if neighbour.get(1) == 'a'
                and self.get_winner_dist_neigh(neighbour) != neighbour.get(2)
                and self.to_be_conquered(neighbour)])
    ...
    def ask_dist(self, dist, voter):
        ...
        if dist.conquer and voter.get(3) != 'a' and voter.get(3) == self.get_winner_dist(dist) and (self.calc_score_dist(dist, 'a') <= (self.calc_score_dist(dist, 'b')+2) or self.calc_score_dist(dist, 'a') <= (self.calc_score_dist(dist, 'c')+2)):
            permission = False
        return permission
...
class Copeland(Rule):
    ...
    def gerry(self):
        conquer = self.can_conquer()
        scores_in_dists = np.array([self.calc_score_dist(dist, 'ab') + self.calc_score_dist(dist, 'ac') for dist in self.grid.dist_list])
        ranks = scores_in_dists.argsort()[::-1]
        for i in range(conquer):
            self.grid.dist_list[ranks[i]].set_conquer(True)
        first_dist = random.randint(0, len(self.grid.dist_list) - 1)
    ...
    def get_neighbours_by_type(self, neighbours, dist_no, last_dist_no):
        ...
        if dist.get_conquer():
            ...
            neighbours_by_type.append([neighbour for neighbour in neighbours if neighbour.get(1) == 'a' 
                and self.to_be_conquered(neighbour)])
            neighbours_by_type.append([neighbour for neighbour in neighbours if neighbour.get(2) == 'a'
                and neighbour.get(1) != self.get_winner_dist(dist)
                and (not self.to_be_conquered(neighbour))])
            neighbours_by_type.append([neighbour for neighbour in neighbours if neighbour.get(2) == 'a'
                and neighbour.get(1) != self.get_winner_dist(dist)
                and self.to_be_conquered(neighbour)])
    ...
    def results(self):
        ...
        dist_percentage = round(len(conquered_districts) / float(self.grid.districts), 2)
        sq = self.grid.size * self.grid.size
        percentage = round(Borda.calc_score(self.grid.profile())['a'] / float(sq), 2)
        print('the gerrimanderer has conquered ' + str(len(conquered_districts)) + ' districts out of ' + str(self.grid.districts) 
                    + ' when ' + str(self.can_conquer()) + ' were possible')
        print('the gerrimanderer has achieved a percentage of ' + str(dist_percentage) + ' instead of ' + str(percentage))
    ...
    def ask_dist(self, dist, voter):
        permission = True
        if dist.conquer and voter.get(1) == 'a' and (self.calc_score_dist(dist, 'ab') <= 1 or self.calc_score_dist(dist, 'ac') <= 1):
            permission = False
        # this second condition ensures that if the voter ranks the non-a alternative that is winning the district last he does not get removed from the district
        if dist.conquer and voter.get(3) != 'a' and voter.get(3) == self.get_winner_dist(dist) and (self.calc_score_dist(dist, 'ab') <= 1 or self.calc_score_dist(dist, 'ac') <= 1):
            permission = False
        return permission

    # returns the alternative that is more likely to win the district under copeland
    def get_winner_dist(self, dist):
        copeland = self.calc_score(dist.voters)
        alt_scores = {}
    ...
    def victory_dist(self, dist):
        victory = False
        if self.calc_score_dist(dist, 'ab') > 0 and self.calc_score_dist(dist, 'ac') > 0:
            victory = True
        return victory
